...
import { api, SHOWCASE_NON_OBSERVED } from './lib/helpers'
...
function App() {
  ...
  const saveLeague = async () => {
    ...
    try { const { data } = await api.put('/config', { league: leagueDraft }); const saved = data?.league || leagueDraft; setConfiguredLeague(saved); setLeagueDraft(saved); setSelectedLeague(saved); setMigrationRequired(false); setLeagueMessage('Shared league saved. Collector will use it on its next cycle.') }
  ...
  return <div className="app-shell">
    ...
    {SHOWCASE_NON_OBSERVED && <div className="boot-warning" role="status">SHOWCASE · non-observed values for a UI walkthrough. Not live market evidence.</div>}
