...
export function CFOTab({ leagues, selectedLeague, setSelectedLeague }) {
  ...
  async function loadPortfolio(id) {
    ...
    } catch (err) { localStorage.removeItem('deuscfo.paperPortfolioId'); setPortfolioId(''); setPortfolioError(err.response?.data?.detail || 'Paper portfolio unavailable') }
    finally { setPortfolioLoading(false) }
  }
  ...
  return <div className="terminal-page">
    <div className="page-head"><div><div className="eyebrow">CAPITAL OFFICE / PLAN</div><h1>CFO</h1><p className="muted">Decision support first. No execution occurs in OBSERVE mode. Bankroll values are Divine.</p></div><div className="market-state"><span className="status-dot" /> MARKET STATE <strong>{plan ? (plan.positions?.length ? 'SELECTIVE' : 'QUIET') : 'UNASSESSED'}</strong></div></div>
    <PlanForm {...{ leagues, selectedLeague, setSelectedLeague, mode, setMode, hours, setHours, simulations, setSimulations, bankroll, updateBankroll, preferences, updatePreference, presets, applyPreset, advanced, setAdvanced, loading, runPlan, portfolioId, usePaperBankroll, setUsePaperBankroll }} />
    {loading && <LoadingState text="Evaluating validated opportunities…" />}{error && <ErrorState message={error} />}{plan && !loading && <PlanView plan={plan} allocation={allocation} expected={expected} probability={probability} portfolioId={portfolioId} addToPaper={addToPaper} />}{!plan && !loading && !error && <div className="terminal-panel empty-panel"><div className="eyebrow">NO PLAN RUN</div><p>Enter a Divine bankroll and run OBSERVE to see whether validated opportunities clear the capital policy.</p></div>}
...
function PlanForm({ leagues, selectedLeague, setSelectedLeague, mode, setMode, hours, setHours, simulations, setSimulations, bankroll, updateBankroll, preferences, updatePreference, presets, applyPreset, advanced, setAdvanced, loading, runPlan, portfolioId, usePaperBankroll, setUsePaperBankroll }) {
  return <form className="terminal-panel plan-form" onSubmit={runPlan}>
    <div className="form-row form-row-main"><Field label="League"><select className="input" value={selectedLeague} onChange={(e) => setSelectedLeague(e.target.value)}>{leagues.map((league) => <option key={league.id} value={league.id}>{league.name}</option>)}</select></Field><Field label="Plan mode"><select className="input" value={mode} onChange={(e) => setMode(e.target.value)}><option>OBSERVE</option><option>PAPER</option><option>AGGRESSIVE-PAPER</option><option>LIVE-CANDIDATE</option></select></Field><Field label="Horizon (hours)"><input className="input numeric" type="number" min="1" value={hours} onChange={(e) => setHours(e.target.value)} /></Field><Field label="Simulations"><input className="input numeric" type="number" min="1" max="10000" value={simulations} onChange={(e) => setSimulations(e.target.value)} /></Field></div>
...
function PlanView({ plan, allocation, expected, probability, portfolioId, addToPaper }) { const honest = plan.mode === 'OBSERVE' && plan.recommendation === 'WAIT'; return <div className="plan-output animate-fade-in"><section className={`decision ${plan.recommendation === 'DEPLOY' ? 'decision-deploy' : 'decision-wait'}`}><div><div className="eyebrow">RECOMMENDATION</div><div className="decision-word">{plan.recommendation}</div><p>{honest ? 'OBSERVE mode is intentionally non-executing. No validated opportunity currently clears the evidence and constraint gates.' : plan.reason}</p></div><div className="decision-meta">{plan.mode_downgraded && <span>requested mode <strong>{plan.requested_mode}</strong></span>}<span>effective mode <strong>{plan.mode}</strong></span><span>objective <strong>{title(plan.objective)}</strong></span></div></section><section className="metric-grid"><Metric label="Net worth" value={divine(plan.bankroll?.total_net_worth)} /><Metric label="Deployed" value={divine(plan.deployed)} tone={plan.deployed ? 'positive' : ''} /><Metric label="Reserve" value={divine(plan.reserve)} /><Metric label="Expected profit" value={divine(expected)} tone={expected > 0 ? 'positive' : ''} /><Metric label="Portfolio probability" value={percent(probability)} /><Metric label="Market state" value={plan.positions?.length ? 'SELECTIVE' : 'WAIT'} /></section><div className="output-grid"><section className="terminal-panel"><PanelTitle>Recommended actions <span>{plan.positions?.length || 0}</span></PanelTitle>{plan.positions?.length ? <PositionTable positions={plan.positions} portfolioId={portfolioId} addToPaper={addToPaper} /> : <EmptyCopy text="No position is recommended. Watchlist and rejected records remain visible below so the decision stays traceable." />}</section><section className="terminal-panel"><PanelTitle>Outcome distribution</PanelTitle><OutcomeDistribution simulation={plan.simulation} /></section></div><div className="output-grid"><section className="terminal-panel"><PanelTitle>Allocation · Divine</PanelTitle><div className="allocation-bar"><i className="alloc-deployed" style={{ width: `${allocation.deployed}%` }} /><i className="alloc-reserve" style={{ width: `${allocation.reserve}%` }} /><i className="alloc-free" style={{ width: `${allocation.free}%` }} /></div><div className="allocation-legend"><span><b className="legend deployed" /> Deployed {divine(plan.deployed)}</span><span><b className="legend reserve" /> Reserve {divine(plan.reserve)}</span><span><b className="legend free" /> Unallocated {divine(plan.unallocated)}</span></div></section><Watchlist watchlist={plan.watchlist} /></div><details className="progressive"><summary>Reasoning and raw plan statistics</summary><div className="raw-grid"><div><h3>Objective components</h3>{Object.entries(plan.objective_components || {}).map(([key,value]) => <div className="raw-row" key={key}><span>{title(key)}</span><strong>{typeof value === 'number' ? value.toFixed(4) : value}</strong></div>)}</div><div><h3>Opportunity tiers / rejected</h3>{Object.entries(plan.opportunity_tiers || {}).map(([key,value]) => <div className="raw-row" key={key}><span>{key}</span><strong>{value}</strong></div>)}{Object.entries(plan.rejected || {}).slice(0,8).map(([key,value]) => <div className="raw-row" key={key}><span>{key}</span><strong>{title(value)}</strong></div>)}</div></div></details></div> }
