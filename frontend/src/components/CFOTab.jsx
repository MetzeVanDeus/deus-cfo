...
import { api, fmtTime, hasPositiveChaosPerDivine } from '../lib/helpers'
...
const modeCopy = (mode) => {
  if (mode === 'OBSERVE') return 'OBSERVE returns analysis only; positions are never executed.'
  if (mode === 'AGGRESSIVE-PAPER') return 'AGGRESSIVE-PAPER simulates larger positions from reconstructed evidence; it never authorizes live trades.'
  if (mode === 'LIVE-CANDIDATE') return 'LIVE-CANDIDATE never places orders; it only scores opportunities against live evidence gates.'
  return 'PAPER is the safe default. DeusCFO never executes trades; the backend may downgrade mode when evidence is weak.'
}
const NUMBER_PREFERENCES = ['minimum_reserve_percent', 'max_single_position_percent', 'max_category_exposure_percent', 'max_correlated_exposure_percent', 'desired_horizon_hours', 'minimum_reserve_amount']
const readStoredObject = (key, fallback) => { let value; try { value = JSON.parse(localStorage.getItem(key) || 'null') } catch { localStorage.removeItem(key); return { ...fallback } } if (!value || typeof value !== 'object' || Array.isArray(value)) { localStorage.removeItem(key); return { ...fallback } } return { ...fallback, ...value } }
...
export function CFOTab({ selectedLeague }) {
  ...
  const updatePreference = (key, value) => {
    setActivePreset('')
    setPreferences((current) => ({ ...current, [key]: NUMBER_PREFERENCES.includes(key) ? Number(value) : value }))
  }
  const applyPreset = (preset) => { setActivePreset(preset); setPreferences({ ...presets[preset] }) }
  ...
  async function loadJournal() {
    ...
    catch { setJournalError('Recommendation journal unavailable') }
  }
  ...
  return <div className="terminal-page">
    ...
    <PlanForm {...{ selectedLeague, mode, setMode, hours, setHours, simulations, setSimulations, bankroll, updateBankroll, preferences, updatePreference, presets, applyPreset, activePreset, advanced, setAdvanced, loading, runPlan, portfolioId, usePaperBankroll, setUsePaperBankroll }} />
    {loading && <LoadingState text="Evaluating validated opportunities…" />}{error && <ErrorState message={error} />}{plan && !loading && <PlanView plan={plan} allocation={allocation} expected={expected} probability={probability} portfolioId={portfolioId} addToPaper={addToPaper} />}{!plan && !loading && !error && <div className="terminal-panel"><EmptyState eyebrow="NO PLAN RUN" title="No capital plan yet" message="Run PAPER after backfill to see exploratory Currency Exchange ideas alongside any validated capital positions." compact /></div>}
    <PaperPortfolio {...{ plan, bankroll, portfolioId, portfolio, portfolioLoading, portfolioError, createPortfolio, forgetPortfolio, realForm, setRealForm, recordReal, realMessage, trades, manualTrades, realizePosition, correctTrade, realizeMessage }} />
    <RecommendationJournal journal={journal} error={journalError} />
  </div>
}
function PlanForm({ selectedLeague, mode, setMode, hours, setHours, simulations, setSimulations, bankroll, updateBankroll, preferences, updatePreference, presets, applyPreset, activePreset, advanced, setAdvanced, loading, runPlan, portfolioId, usePaperBankroll, setUsePaperBankroll }) {
  return <form className="terminal-panel plan-form" onSubmit={runPlan}>
    <div className="form-row form-row-main"><Field label="Plan mode"><select className="input" value={mode} onChange={(e) => setMode(e.target.value)}><option value="OBSERVE">OBSERVE</option><option value="PAPER">PAPER</option><option value="AGGRESSIVE-PAPER">AGGRESSIVE-PAPER</option><option value="LIVE-CANDIDATE">LIVE-CANDIDATE (no execution)</option></select></Field><Field label="Plan window (hours)"><input className="input numeric" type="number" min="1" value={hours} onChange={(e) => setHours(e.target.value)} /></Field><Field label="Simulations"><input className="input numeric" type="number" min="1" max="10000" value={simulations} onChange={(e) => setSimulations(e.target.value)} /></Field></div>
    ...
    <div className="form-section-label preference-label"><span>PREFERENCES</span><div className="segmented preset-buttons" role="group" aria-label="Risk preference preset">{Object.keys(presets).map((preset) => <button type="button" key={preset} className="segmented-option" aria-pressed={activePreset === preset} onClick={() => applyPreset(preset)}>{preset}</button>)}</div></div>
    <div className="form-row preference-grid"><Field label="Risk"><select className="input" value={preferences.risk_tolerance} onChange={(e) => updatePreference('risk_tolerance', e.target.value)}><option>low</option><option>medium</option><option>high</option></select></Field><Field label="Desired horizon (h)"><input className="input numeric" type="number" min="1" value={preferences.desired_horizon_hours} onChange={(e) => updatePreference('desired_horizon_hours', e.target.value)} /></Field><Field label="Min liquidity"><select className="input" value={preferences.minimum_liquidity} onChange={(e) => updatePreference('minimum_liquidity', e.target.value)}><option>low</option><option>medium</option><option>high</option></select></Field><Field label="Max effort"><select className="input" value={preferences.maximum_effort} onChange={(e) => updatePreference('maximum_effort', e.target.value)}><option>low</option><option>medium</option><option>high</option></select></Field><Field label="Reserve %"><input className="input numeric" type="number" min="0" max="100" value={preferences.minimum_reserve_percent} onChange={(e) => updatePreference('minimum_reserve_percent', e.target.value)} /></Field></div>
    ...
    <div className="form-actions"><span className="muted small">{modeCopy(mode)}</span><button className="btn-primary" disabled={loading || !selectedLeague}>{loading ? 'BUILDING PLAN…' : 'RUN CAPITAL PLAN'}</button></div>
  </form>
}
...
function EmptyCopy({ text }) { return <EmptyState message={text} compact /> }
...
function PaperPositionRow({ position, realizePosition }) {
  ...
  return <>
    <tr>
      <td>{position.opportunity_id}</td>
      <td>{position.status}</td>
      <td className="numeric">{position.quantity}</td>
      <td className="numeric">{chaos(position.entry_price)}</td>
      <td className="numeric">{chaos(position.realized_profit)}</td>
      <td>{position.status === 'open' && <button type="button" className="text-button" aria-expanded={open} onClick={() => setOpen((current) => !current)}>{open ? 'HIDE' : 'REALIZE'}</button>}</td>
    </tr>
    {open && position.status === 'open' && <tr className="detail-row"><td colSpan="6"><form className="real-form" onSubmit={(event) => { event.preventDefault(); realizePosition(position, form) }}><Field label="Actual entry (c)"><input required className="input numeric" type="number" min="0.000001" step="any" value={form.actual_entry_price} onChange={(e) => setForm({ ...form, actual_entry_price: e.target.value })} /></Field><Field label="Actual quantity"><input required className="input numeric" type="number" min="0.000001" step="any" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /></Field><Field label="Actual exit (c)"><input required className="input numeric" type="number" min="0.000001" step="any" value={form.exit_price} onChange={(e) => setForm({ ...form, exit_price: e.target.value })} /></Field><Field label="Actual duration (h)"><input className="input numeric" type="number" min="0" step="any" value={form.actual_duration_hours} onChange={(e) => setForm({ ...form, actual_duration_hours: e.target.value })} /></Field><Field label="Confidence (0–1)"><input className="input numeric" type="number" min="0" max="1" step="any" value={form.confidence} onChange={(e) => setForm({ ...form, confidence: e.target.value })} /></Field><div className="form-actions"><span className="muted small">Calibration only. REALIZE does not place an order.</span><button className="btn-secondary">REALIZE</button></div></form></td></tr>}
  </>
}
