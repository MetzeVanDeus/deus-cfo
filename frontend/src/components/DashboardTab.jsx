...
import { api, fmtRelative, historyWindowLabel } from '../lib/helpers'
import { FlipFinder } from './FlipFinder'
import { LoadingState, EmptyState, ErrorState, LeagueEmpty } from './ui'

export function DashboardTab({ categories, selectedLeague, historyHours = 24 }) {
  const [overview, setOverview] = useState(null)
  const [signals, setSignals] = useState([])
  const [loading, setLoading] = useState(Boolean(selectedLeague))
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      Promise.all([
        ...
        api.get('/signals', { params: { league: selectedLeague, hours: historyHours } }),
  ...
  const snapshotLabel = fmtRelative(snapshot)
