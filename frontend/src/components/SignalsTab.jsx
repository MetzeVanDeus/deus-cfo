...
export function SignalsTab({ selectedLeague, historyHours = 24 }) {
  ...
  useEffect(() => { if (!selectedLeague) return; let cancelled = false; setLoading(true); setError(''); api.get('/signals', { params: { league: selectedLeague, hours: historyHours } }).then((response) => { if (!cancelled) { setSignals(response.data); setLoading(false) } }).catch(() => { if (!cancelled) { setError('Failed to load signals'); setLoading(false) } }); return () => { cancelled = true } }, [selectedLeague, historyHours])
