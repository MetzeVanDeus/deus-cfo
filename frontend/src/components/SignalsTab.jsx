...
function SignalRow({ signal }) {
  ...
  return <>
    ...
    {open && <tr className="detail-row"><td colSpan="6"><div className="position-detail"><span>Time: <strong>{fmtRelative(signal.timestamp || signal.detected_at || signal.created_at)}</strong></span><span>Type: <strong>{signal.type || signal.signal_type || '—'}</strong></span><span>Item: <strong>{signal.item || signal.item_name || '—'}</strong></span><span>Change: <strong>{move == null ? '—' : `${Number(move).toFixed(1)}%`}</strong></span><span>Source: <strong>{signal.source || '—'}</strong></span>{signal.what_happened && <span>What happened: <strong>{signal.what_happened}</strong></span>}{signal.why_it_matters && <span>Why it matters: <strong>{signal.why_it_matters}</strong></span>}{signal.possible_action && <span>Possible action: <strong>{signal.possible_action}</strong></span>}</div><details><summary>Raw payload</summary><pre>{JSON.stringify(signal, null, 2)}</pre></details></td></tr>}
  </>
}
